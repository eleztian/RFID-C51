...
/*	function SwapImg(opts,obj){
	...
	SwapImg.prototype={
		...
		init:function(){
			...
			iconNode.find("li").each(function(i){
				$(this).on("click",function(){
					...
					if($(this).hasClass("current") || self.swapFlag ){
						return false;
					}
					self.swapHandle(i);
				})
		...
		swapHandle:function(i){
			...
			$(self.iconSelector).find("li").removeClass("current").eq(self.index).addClass("current");
			...
			$(self.swapSelector).eq(self.index).animate({opacity:1},self.duration,function(){
				...
				$(this).parent().css({"position":"absolute","z-index":"100","height":"200","top":"0","left":"0"})
			});
